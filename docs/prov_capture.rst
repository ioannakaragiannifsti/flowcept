...
Importing ``DecisionCapture`` makes the API available without changing any model.
To enable automatic decision generation, attach an **unwrapped LangChain model**
using ``llm=`` and call ``invoke`` inside an active ``Flowcept`` context:

.. code-block:: python

   from flowcept import DecisionCapture, Flowcept

   # llm is your configured LangChain model, without tools bound to it.
   with Flowcept(start_persistence=False):
       decision = DecisionCapture(
           decision_type="selection",
           context="Choose the output that best meets the request",
           agent_id="my-agent",
           llm=llm,
       )
       record = decision.invoke("Propose two short greetings and select the more formal one.")
       print(record.to_dict())

This requires the ``llm_agent`` extra and credentials for the attached model.
The runnable ``examples/decision_provenance_example.py`` uses an OpenAI-compatible
model configured through ``agent.model``, ``agent.api_key`` (or ``AGENT_API_KEY``),
and optionally ``agent.llm_server_url`` in your settings. On Windows, provide
``sys_metadata.sys_name`` and ``sys_metadata.node_name`` in those settings.

The capture automatically supplies a generic prompt and the ``response_format``
JSON-schema parameter, then validates the returned
JSON before recording candidates, assessments, and selections. Every candidate
must have an assessment. Scores must be finite numbers in [0, 1] and are stored
as ``model_reported_confidence``: they are model assessments, not calibrated
probabilities. The alternatives are explicitly generated outputs, not a record
of hidden internal reasoning.

The attached model must accept the OpenAI-compatible JSON-schema ``response_format``
parameter (as supported by Ollama's compatible endpoint). Callers do not supply
``response_format`` themselves. Unsupported providers fail visibly; there is no
automatic fallback to prompt-only output. Provider schema restrictions still apply.

``invoke`` returns a ``DecisionRecord`` and also assigns ``decision.record`` and
``decision.task_id``. Use a fresh capture for each decision. Automatic calls cannot
be mixed with manually added candidates in the same capture. The existing manual
``add_candidate``, ``assess``, and ``select`` context-manager API remains available.

The LLM invocation is captured by ``FlowceptLLM``. Its task ID becomes the decision
task's ``parent_task_id``; both carry the same agent and workflow context. Candidate
statuses are derived from the selection, and runtime identifiers are generated by
Flowcept. The graph-compatible payload remains in ``generated.decision``; this
does not create separate graph nodes for every candidate or assessment.

For an already instrumented invocation, use
``decision.record_response(response_json, invocation_task_id=task_id)`` to validate
and record its decision output using the same contract. Malformed output raises
an error without recording a successful decision. There are no automatic retries
or JSON-repair fallbacks. A completed LLM call remains recorded even when its
decision output fails validation. Only synchronous explicit decision calls are
supported; automatic interception of tool calls, streaming, and LangGraph loops
is not enabled by this API.
